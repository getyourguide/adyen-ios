...
public func localizedSubmitButtonTitle(with amount: Amount?,
    ...
    if let amountValue = amount?.value, amountValue == 0 {
    ...
    case .immediate:
        guard let value = amount?.value, let currencyCode = amount?.currencyCode else {
            return localizedString(.submitButton, parameters)
        }

        let localeIdentifier = amount?.localeIdentifier ?? parameters?.locale
        let formattedAmount = Amount(value: value, currencyCode: currencyCode, localeIdentifier: localeIdentifier).formatted
